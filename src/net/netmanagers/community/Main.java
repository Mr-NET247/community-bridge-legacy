...
public class Main extends JavaPlugin {
	...
	public static boolean create_players = false;
	...
	public void onEnable() {
		...
		if (config.get("db-username").equals("username") && config.get("db-password").equals("password")) {
			...
			create_players = config.getBoolean("create-players");
			...
			if(show_config){
				...
				Main.log.info("Create Players : " + create_players);
	...
	public static int makeUserId(String username) {
		int userId = 0;
		try {
			if (multi_tables) {
				// Not Yet Supported
			} else {				
				sql.insertQuery("INSERT INTO " + users_table + " (`"+user_name_field+"`) VALUES ('" + username + "')");
	...
	public static void SyncPlayer(Player p, boolean firstsync){		
		try {
			...
			if(id < 1 && create_players){
				log.info(p.getName() + " is a new player creating empty user");
				id = makeUserId(p.getName());
				log.info(p.getName() + " miner id is " + id);
			}
